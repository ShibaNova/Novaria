...
contract PlaceManager is Editor {
            ...
            for (uint i = 0; i < stars.length; i++) {
                ...
                for (uint j = 0; j < jackpots.length; j++) {
                    if (jackpots[j].starId == i) {
                        uint _numerator = _allocation * jackpots[j].starDist;
                        uint _denominator = jackpotTotal[i];
                        jackpots[j].availableNova = (_numerator > 0 ?
                            jackpots[j].availableNova + (_numerator / _denominator) : 0);
    ...
    function refine() external {
        ...
        require(getUserUNova(_sender) > 0, "PLACEMANAGER: Player has no unrefined Nova");
        uint _total;
        for (uint i=0; i < stars.length; i++) {
        ...
        previousBalance = previousBalance - _total;
        emit NovaRefined(_sender, _total);
    }
